<section id="accordion" class="max-w-6xl mx-auto py-12 sm:max-w-sm md:max-w-2xl">
    ...
    {{-- <div id="accordion-collapse" data-accordion="collapse">
        @foreach ($questions as $index => $question)
            <h2 id="accordion-collapse-heading-{{ $index + 1 }}">
                <button type="button" class="flex items-center justify-between w-full p-5 font-medium rtl:text-right text-gray-500 border {{ $index === 0 ? 'border-b-0' : '' }} border-gray-200 {{ $index === 0 ? 'rounded-t-xl' : '' }} focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 gap-3" data-accordion-target="#accordion-collapse-body-{{ $index + 1 }}" aria-expanded="{{ $index === 0 ? 'true' : 'false' }}" aria-controls="accordion-collapse-body-{{ $index + 1 }}">
                    <span>{{ $question }}</span>
    ...
    <div class="mt-8" id="accordion-flush" data-accordion="flush">
        @foreach ($questions as $index => $question)
